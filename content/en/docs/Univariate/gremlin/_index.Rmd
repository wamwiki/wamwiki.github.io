...
model1.1 <- gremlin(birth_weight ~ 1, #Response and Fixed effect formula
...
It is always a good idea to ensure the REML has converged on the maximum likelihood. This is printed to the screen, but we can also retrieve whether or not the model has converged from the model object `model1.1`

```{r, cache = FALSE, message = FALSE}
ccFun(model1.1)  #<-- TRUE if all convergence checks pass
```
...
model1.2 <- update(model1.1, maxit = 50)
```
